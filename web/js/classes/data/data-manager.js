...
class DataManager extends EventEmitter {
    ...
    deleteItem(id) {
        if (config.debug) console.log("delete", id)
    }

    addItem(item) {
        if (config.debug) console.log("add", item)
        this._data.put(item[this._keyField], item)
    }
    saveNewItem(item){
    ...
    hasPendingChanges() {
        if (config.debug) console.log(this._updateQ.size)
    ...
    setupListeners() {
        ...
        self._comms.on('list',list =>{
            if(config.verbose) console.log("list event", list.length)
        ...
        self._comms.on('item.updated', item => {
            if(config.verbose) console.log("item.updated", e.detail.item)
        ...
        self._comms.on('item.edit', evt => {
            if(config.verbose) console.log("item.edit event cap", evt)
